...
module.exports = function(input){
    ...
    function find(substring, rendition){
        if(type(rendition, "string")){
          return match(substring, rendition)
        }
        else if(type(rendition, "array")){
          return findOne(substring, rendition);
        }
    }

    function findOne(substring,rendition){
      var found = false
      each(rendition, function(element, i){
        if(match(substring, element)){
          found = true
        }
      })
      return found
    ...
    function findAll(substring, rendition){
      var count = 0
      each(rendition, function(element, i){
        if(match(substring, element)){
          count++
    ...
    function match(substring, element){
        if(type(substring, "regexp")){
          return substring.test(element)
        }
        else if(type(substring, "string")){
