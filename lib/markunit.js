...
module.exports = function(input) {
  var converter = new MarkdownIt({ html: true });
  ...
  function extractCopy(data) {
    ...
    each(data, function(element, i) {
      var subdata = cheerio.load(element).text();
  ...
  function extractCode(data) {
    var markup = cheerio.load(data);
    var array = [];
    markup("code").each(function(index, element) {
      array.push(markup(element).text());
    });
    return array;
  }
